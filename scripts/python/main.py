...
def main() -> int:
    ...
    parser.add_argument("--skip-staging", action="store_true", help="Skip staging load.")
    parser.add_argument("--skip-models", action="store_true", help="Skip warehouse models.")
    parser.add_argument("--skip-quality", action="store_true", help="Skip SQL data-quality loops.")
    parser.add_argument("--skip-gx", action="store_true", help="Skip Great Expectations gate.")
    parser.add_argument(
        "--continue-on-error", action="store_true", help="Continue past failures (still exits 1 at end)."
    )
    parser.add_argument("--help-stages", action="store_true", help="Print stage descriptions and exit.")
    args = parser.parse_args()

    if args.help_stages:
        print("Pipeline stages:")
        print("  1. pg_staging.py            — MongoDB -> Postgres staging (incremental, validated, quarantine_log)")
        print("  2. run_models.py            — dims + facts in core (SCD2 dim_customers, as-of fact_orders, logged per model)")
        print("  3. run_data_quality_loops.py — 5 read-only SQL loops (catalog-driven, --strict)")
        print("  4. gx_run.py                — Great Expectations suites (read-only, --strict)")
        print("\nAll four stages are enabled by default. Use --skip-* to disable.")
        print("Observability: each stage writes to core.pipeline_run_log (run_id per invocation).")
        return 0

    run_id = uuid.uuid4()
    _log.info("Pipeline run_id=%s started (continue_on_error=%s)", run_id, args.continue_on_error)
    ...
    if not args.skip_models:
        stages.append(("MODELS", SCRIPTS_DIR / "run_models.py", ["--continue-on-error"] if args.continue_on_error else []))
            ...
            with eng.begin() as conn:
                ...
                conn.execute(
                    ...
                    ),
                    {
                        ...
                        "error": f"{len(failures)} stage(s) failed: {', '.join(lbl for lbl, _ in failures)}"[:1000],
                    },
                )
        except Exception:
            _log.warning("Failed to write pipeline aggregate to pipeline_run_log — ignored.", exc_info=True)
        return 1
    ...
    try:
        ...
        _log.warning("Failed to write pipeline PASS aggregate — ignored.", exc_info=True)
