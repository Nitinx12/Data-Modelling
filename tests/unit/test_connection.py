...
class TestGetMongoDb:
    def test_creates_mongo_client_on_first_call(self, mock_env: dict[str, str]) -> None:
        with (
...
class TestGetDatabricksConnection:
    @pytest.fixture
    def dbx_env(
        self, mock_env: dict[str, str], clean_env: pytest.MonkeyPatch
    ) -> dict[str, str]:
        for k, v in {
    ...
    def test_creates_databricks_connection_on_first_call(
        ...
        with (
            patch("databricks.sql.connect") as mock_connect,
            patch("utils.connection.get_logger", return_value=MagicMock()),
        ):
            mock_conn = MagicMock()
