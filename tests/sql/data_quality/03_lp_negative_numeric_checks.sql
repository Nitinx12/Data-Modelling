-- ============================================================================
-- Loop 3 of 5: numeric columns must not contain negative values.
...
BEGIN
    ...
    LOOP
        ...
        LOOP
            EXECUTE format(
                'SELECT COUNT(*) FROM %I.%I AS t WHERE t.%I < 0'
                , v_table.schema_name
                , v_table.table_name
            ...
            IF v_failed_rows > 0 THEN
                ...
                RAISE NOTICE '[FAILED] %.%.% has % negative value(s).'
                    , v_table.schema_name
                    , v_table.table_name
                    , v_column.column_name
                    , v_failed_rows;
    ...
    RAISE NOTICE 'Negative-numeric loop complete: % failed check(s), % failed row(s).'
        , v_failed_checks
        , v_total_failed_rows;
END;
$$;
